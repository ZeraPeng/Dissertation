...
def xprompt_check():
    ...
    root_path = "/usr1/home/s124mdg53_04/Dissertation/ASKG"  # Set your root directory path here
    label_file = os.path.join(root_path, 'data/ntu', 'ntu120_label.txt')
    xprompt_file = os.path.join(root_path, 'data/vocab', "classes_xprompt_ntu_o.yml")
    output_file = os.path.join(root_path, 'data/vocab', "classes_xprompt_ntu.yml")
    ...
    print(f"Remaining labels: {list(set(labels)-set(new_labels))}")
    with open(output_file, 'w', encoding='utf-8') as f:
        yaml.dump(xprompt, f, allow_unicode=True, sort_keys=False, default_flow_style=False)

def xprompt_format(dataset_name='ntu'):
    root_path = "/usr1/home/s124mdg53_04/Dissertation/ASKG"
    xprompt_file = os.path.join(root_path, 'data/vocab', "classes_xprompt_ntu.yml")
    output_file = os.path.join(root_path, 'data/vocab', "classes_xprompt_ntu_f.yml")
    with open(xprompt_file, 'r') as f:
