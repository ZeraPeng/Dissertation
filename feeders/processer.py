...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.action_descriptions = torch.load('/usr1/home/s124mdg53_04/Dissertation/text_feature/ntu_semantic_part_feature_dict_gpt35_6part.tar')
        # _ = torch.load('/usr1/home/s124mdg53_04/Dissertation/text_feature/ntu_spatial_attribute_feature_dict_gpt35.tar')
        self.attribute_features_dict = torch.load('/usr1/home/s124mdg53_04/Dissertation/text_feature/ntu_semantic_part_feature_dict_gpt35_6part.tar')
    ...
    def load_model(self):
        ...
        # self.log_softmax = nn.LogSoftmax(dim=1).cuda(output_device)
        ...
        # self.model_image_dict = nn.ModuleDict()
        # self.model_image_preprocess = dict()

        # for name in self.arg.model_args['head']:
        #     # model_, preprocess = clip.load(name, output_device)
        #     # model_, preprocess = clip.load('ViT-L/14', device)
        #     # del model_.visual
        #     # model_image = ImageCLIP(self.arg.model_args['head'])
        #     model_text = TextCLIP(self.arg.model_args['head'])
        #     # model_image = model_image.cuda(self.output_device)
        #     model_text = model_text.cuda(self.output_device)
        #     self.model_text_dict[name] = model_text
        #     # self.model_image_dict[name] = model_image
        #     # self.model_image_preprocess[name] = preprocess
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):          
            ...
            label_language = torch.cat([self.action_descriptions[0][l.item()].unsqueeze(0) for l in label], dim=0).cuda(self.output_device)
            all_label_language = self.action_descriptions[0].cuda(self.output_device)[:60]
            # label_language_new = []
            ...
            # label_language_seen = label_language_new[seen_classes]
            # label_language = torch.cat([label_language_new[l.item()].unsqueeze(0) for l in label], dim=0).cuda(self.output_device)
            # all_label_language = label_language_new[:60]
            # label_language = torch.cat(label_language, dim=1).cuda(self.output_device)[seen_classes]
            ...
            print("Saving extracted part features.")
            print(f"Shape of part visual feature: {part_visual_feature.shape}")
            np.save('/usr1/home/s124mdg53_04/STAR/part_sk_feats/part_visual_feature.npy', part_visual_feature.detach().cpu().numpy())

            # loss_part_infonce, loss_global_ce, loss_calibration_unseen = self.model.loss_cal(part_visual_feature, global_visual_feature, part_language, label_language, true_label_array, all_label_language,unseen_classes)
            # loss_global_ce, loss_cvae_reconstruction, loss_cvae_kld,loss_classification, loss_mse_align, loss_align_lpd, loss_align_spd, loss_prompt, loss_global_ce_semantic, loss_kl_divergence, loss_calibration_unseen, loss_part_ce, loss_pd_label, loss_classification_unseen, loss_ske_ce = self.model.loss_cal(part_visual_feature, global_visual_feature, part_language, part_language_seen,part_language_seen_unseen, label_language_seen,
            ...
            # loss_value_img.append(loss_img.data.item())
            # loss_value_text.append(loss_text.data.item())
            # loss_value_img_mse.append(loss_img_mse.data.item())
            # loss_value_mse.append(loss_mse.data.item())
            # loss_value_cross_modal_align.append(loss_cross_modal_align.data.item())
            ...
            # self.train_writer.add_scalar('loss_part_infonce', loss_part_infonce.data.item(), self.global_step)
            # self.train_writer.add_scalar('loss_img', loss_img.data.item(), self.global_step)
            # self.train_writer.add_scalar('loss_text', loss_text.data.item(), self.global_step)
            # self.train_writer.add_scalar('loss_img_mse', loss_img_mse.data.item(), self.global_step)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    # embedding_dict, all_embedding_feature = self.model(data, ntu_spatial_temporal_attribute_feature_dict)
                    part_language = []
                    # label_language = []
                    for i, part_name in enumerate(["head", "hand", "arm", "hip", "leg", "foot"]):
                        part_language.append(self.action_descriptions[i+1].unsqueeze(1))
                    ...
                    if ln == 'test_gzsl':
                        label_language = self.action_descriptions[0].cuda(self.output_device)[:60,:]
                        sample_label_language = torch.cat([self.action_descriptions[0][l.item()].unsqueeze(0) for l in label], dim=0).cuda(self.output_device)
                        # label_language_new = []
                        # for i, part_name in enumerate(["al","head", "hand", "arm", "hip", "leg", "foot"]):
                        #     label_language_new.append(self.action_descriptions[i])
                        ...
                        # sample_label_language = torch.cat([label_language_new[l.item()].unsqueeze(0) for l in label], dim=0).cuda(self.output_device)
                        part_language_unseen_seen = part_language1[:60]
                        part_language_seen = part_language1[:60,:]
                        ...
                        # loss_part_infonce, loss_global_ce = self.model.loss_cal(part_visual_feature, global_visual_feature, part_language, label_language, true_label_array)
                        # loss = loss_part_infonce +loss_global_ce
                        gzsl_feature.append(global_semantic)
                        gzsl_label.append(label)
            ...
            if ln == 'test_gzsl':
                ...
                # accuracy_seen_list, accuracy_unseen_list = self.model.get_gzsl_acc2(re_loss, label_list_acc.tolist(), sim_matrix, unseen_classes, percentage_factor_list, class_prob)
                # class_prob = []
                # acc_result = self.model.get_gzsl_acc3(re_loss, label_list_acc.tolist(), sim_matrix, unseen_classes, [0.45], class_prob,[i/100 for i in range(5, 100, 5)])# print(tmp_print)
                acc_result = self.model.get_gzsl_acc(re_loss, label_list_acc.tolist(), sim_matrix, unseen_classes, [0.45], class_prob,[i/100 for i in range(5, 100, 5)], [0, 0.001, 0.005]+[i/10000 for i in range(100, 160, 10)]+[i/1000 for i in range(20,100,5)] + [0.25, 0.5, 0.75])  # ntu60 xsub 55/5
